...
import {
  ...
  getPieChartDataSet,
  constructInitDashboardDataset, getLineChartDataSet,
} from './index';
...
export default {
  ...
  methods: {
    ...
    onCurrencyChange(checkedValues) {
      const choose = checkedValues;
      // eslint-disable-next-line prefer-destructuring
      this.startDate = choose[0];
      // eslint-disable-next-line prefer-destructuring
      this.endDate = choose[1];
      axios.get('http://localhost:8080/findEverydayBusiness',{params:{startDate:this.startDate,endDate:this.endDate},headers: {'jwtToken': localStorage.getItem('jwtToken')}}).then(result=>{
        console.log(result.data);
        const array=[];
        for (let i = 0; i < result.data.data.length ; i++) {
          array.push(result.data.data[i].money);
        }
        console.log(array);
        const dataUser = JSON.stringify(array)
        sessionStorage.setItem("useronlineTotalpay",dataUser)
        console.log(`转成JSON字符串后的数据是${dataUser}`);
      })
    ...
    findAllData(){
      axios.get('http://localhost:8080/findAllData',{params:{},headers: {'jwtToken': localStorage.getItem('jwtToken')}})
        .then(result=>{
          console.log(result.data);
          PANE_LIST[0].number = result.data.data.earning;
          PANE_LIST[1].number = result.data.data.refund;
          PANE_LIST[2].number = result.data.data.vips;
          PANE_LIST[3].number = result.data.data.orders;
        })
    },
    /**
     * 初始化每日营业额表格
     */
    initializea(){
      axios.get('http://localhost:8080/findEverydayBusiness',{params:{},headers: {'jwtToken': localStorage.getItem('jwtToken')}}).then(result=>{
        console.log(result.data);
        const array=[];
        for (let i = 0; i < result.data.data.length ; i++) {
          array.push(result.data.data[i].money);
     ...
     */
    findOrdersBySole(){
      axios.get('http://localhost:8080/findOrdersBySole',{params:{},headers: {'jwtToken': localStorage.getItem('jwtToken')}}).then(result=>{
        console.log(result.data)
        for (let i = 0; i < 6; i++) {
          SALE_TEND_LIST[i].productName = result.data.data[i].hotelname;
          SALE_TEND_LIST[i].count = result.data.data[i].num;
        }
      })
    },
    /**
     ...
     */
    findAccruing(){
      axios.get('http://localhost:8080/useronline/accruing',{params:{},headers: {'jwtToken': localStorage.getItem('jwtToken')}}).then(result=>{
        console.log(result.data);
        // const array=[];
        for (let i = 0; i < 6; i++) {
          BUY_TEND_LIST[i].productName = result.data.data[i].username;
          BUY_TEND_LIST[i].count = result.data.data[i].useronlineTotalpay;
          // array.push(result.data.data[i].useronlineTotalpay);
        }
